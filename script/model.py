...
class Actor(object):
    def __init__(self,
        ...
        with tf.variable_scope('actor'):
            ...
            self.lengths = tf.placeholder(tf.int32, [None], name='lengths') # b
        ...
        print self.action_gradient.get_shape().as_list()
        self.actor_gradients = tf.gradients(self.a_online, self.network_params, -self.action_gradient)
    ...
    def Model(self):      
        ...
        print 'depth vector shape', shape
    ...
    def Predict(self, depth_input, lengths, prev_rnn_state_online):
        return self.sess.run([self.a_test, self.rnn_state_online], feed_dict={
            self.depth_input: depth_input,
            self.lengths: lengths,
...
class Critic(object):
    def __init__(self,
        ...
        self.square_diff = tf.pow((self.predicted_q - self.q_online), 2)

        self.loss_t = tf.reduce_sum(self.square_diff, reduction_indices=1) / tf.cast(self.lengths, tf.float32)
    ...
    def Model(self):
        ...
        # training
        depth_vectors = tf.reshape(conv3, (self.batch_size, self.max_steps, shape[1]*shape[2]*shape[3])) # b, l, h*w*c
        action_input_reshape = tf.reshape(self.action_input, (self.batch_size, self.max_steps, 2)) # b, l, 2
        ...
        # testing
        depth_vectors_test = tf.reshape(conv3, (1, 1, shape[1]*shape[2]*shape[3])) # b, l, h*w*c
        action_input_reshape_test = tf.reshape(self.action_input, (1, 1, 2)) # b, l, 2
    ...
    def Precit(self, depth_input, action_input, prev_rnn_state_online, lengths):
        return self.sess.run([self.q_test, self.rnn_state_online], feed_dict={
            self.depth_input: depth_input,
            self.action_input: action_input,
            self.prev_rnn_state_online: prev_rnn_state_online,
            self.lengths: lengths
